...
yuzhi = 0.8
logging.basicConfig(filename=f'yuzhi_{yuzhi}_GraphSage_experiment.log', level=logging.INFO)
...
class Graphsage(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, hybrid_sample_neighbors):
        super(Graphsage, self).__init__()
        self.conv1 = SAGEConv(in_channels, hidden_channels, hybrid_sample_neighbors)
        self.conv2 = SAGEConv(hidden_channels, out_channels, hybrid_sample_neighbors)
...
def hybrid_sample_neighbors(edge_index, num_nodes, num_samples=10):
    alpha = torch.rand(1).item()
    if alpha > 1 - alpha:
        return weighted_sample_neighbors(edge_index, num_nodes, num_samples, similarity_matrix=similarities)
    else:
        return random_sample_neighbors(num_nodes, num_samples)

def random_sample_neighbors(num_nodes, num_samples):
    sample_idx = torch.randint(0, num_nodes, (num_samples,), dtype=torch.long)
    return sample_idx

def weighted_sample_neighbors(edge_index, num_nodes, num_samples, similarity_matrix):
    row, col = edge_index
    sampled_neighbors = []
    ...
    for node in range(num_nodes):
        ...
        sampled_neighbors.append(top_k_neighbors)
...
for test_ratio in test_ratios:
    ...
    for exp in range(num_experiments):
        ...
        model = Graphsage(in_channels=num_features, hidden_channels=64, out_channels=num_classes,
                          hybrid_sample_neighbors=hybrid_sample_neighbors).to(device)
